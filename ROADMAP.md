...
11,075 tests, 229 conformance programs, 42 examples, 14 spec chapters.  [KNOWN_ISSUES.md](KNOWN_ISSUES.md) tracks the open bugs — burndown material rather than stage work — plus the *limitations* the stages below retire.
...
**Modules and ecosystem** — [#1304](https://github.com/aallan/vera/issues/1304) settle which import supplies a bare name when two of them do — spec §8.5 defines no order, and the checker's pick is a set-iteration artefact that flips between runs of the identical program rather than an order at all ([measured](https://github.com/aallan/vera/issues/1304#issuecomment-5289022950): six consecutive `vera check` runs of one file, stable under a fixed `PYTHONHASHSEED` but tracking module-name hash order rather than import position), with E608 refusing the shape at compile so the flap is observable only at `vera check`, [#187](https://github.com/aallan/vera/issues/187) module-qualified call disambiguation → [#127](https://github.com/aallan/vera/issues/127) module re-exports, [#130](https://github.com/aallan/vera/issues/130) package system and registry, [#163](https://github.com/aallan/vera/issues/163) standalone WASM runtime package, [#238](https://github.com/aallan/vera/issues/238) Component Model interop, [#56](https://github.com/aallan/vera/issues/56) incremental compilation, [#294](https://github.com/aallan/vera/issues/294) effect row variable unification, [#785](https://github.com/aallan/vera/issues/785) GitHits MCP (bookmark; trial at the next dependency-facing milestone).
