...
The handler has to name the same effect *instance*, type arguments
included: `handle[State<Nat>]` does not discharge `State<Int>`, so a
caller around it still gets the row. Propagation stops at the file
boundary, by design: module-qualified calls are not followed.
