...
**`old` and `new` are not available as function names (E153)**: both are contract state forms, so `old(...)` / `new(...)` in expression position always parses as a reference to an effect's before/after state (Chapter 7, Section 7.9.2) and never as a call. A `fn old` or `fn new` — top-level, `where`-helper, or in an imported module — could therefore never be called, and is rejected at `vera check`. Rename it. Only the exact identifiers are reserved: `older`, `renew`, and the like are ordinary function names.
