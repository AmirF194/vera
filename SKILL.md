...
| Generic instantiated inside an **imported** fn body | An imported non-generic function whose body calls a generic — its own nested `forall<T>` helper, or a generic it imports from a deeper module — fails `vera compile` with an internal unknown-func error: imported bodies never get generic-instantiation discovery, so the clone is never emitted and the body is dropped. | No clean source-level workaround (hoisting the helper does NOT help — the parent's imported body is still never scanned). Restructure so the *importing program* instantiates the generic directly at the needed type, or make the parent generic too and hoist the helper to a top-level `public` module generic (a generic parent's clone body IS scanned). | [#999](https://github.com/aallan/vera/issues/999) |
| Generic helper under a **generic** ancestor | A `forall` helper anywhere under a generic ancestor (a generic helper's own generic child, or a generic under a plain helper under a generic grandparent) is hoisted as a still-generic template — clone hoisting substitutes only the ancestor's type vars — and `vera compile` fails with an internal unknown-func error on a check-green, verify-green program. | Give the inner helper the ancestor's type variable instead of its own `forall` (a T-dependent plain helper monomorphizes per-clone, #904), or hoist the inner generic to a top-level `private fn`. | [#1002](https://github.com/aallan/vera/issues/1002) |
| Opaque `unknown func` after a skip | When codegen skips a *called* function (unsupported construct), the compile error shows only `unknown func $f` — the E602 warning explaining the skip is suppressed on the error path. | Compile the suspect function alone (as the only export) to see the real E602 skip reason. | [#1004](https://github.com/aallan/vera/issues/1004) |
| `@Unit` param read in a `get` clause | Reading the `get` clause's `@Unit` op parameter passes `check` but fails codegen with an E699 internal error. | Don't reference the `@Unit` parameter — it carries no value; read the state slot (`@T.0`) instead. | [#1005](https://github.com/aallan/vera/issues/1005) |
| Effect op in an array literal / `let @Unit` | `[get(()), 1]` or `let @Unit = put(x)` compiles to a skipped function (opaque `unknown func` if called). | Bind the op result first (`let @Int = get(());`) and use the slot in the array; call `put(...)` as a statement, not a `let`. | [#1006](https://github.com/aallan/vera/issues/1006) |
| Imported fn builds its module's own ADT | `import lib(make)` where `make` constructs lib's `Inner` fails compile with `unknown func $make` — the ADT's layouts weren't registered. | Add the type to the import list (`import lib(make, Inner);`) even though the importer never names it. | [#1008](https://github.com/aallan/vera/issues/1008) |
| Negative `@Int` into an `apply_fn` `@Nat` formal | `apply_fn(f, <negative>)` where `f` takes `@Nat` verifies clean and stores the negative — the narrowing obligation is missing at this site. | Bind the value into a `@Nat` slot first (`let @Nat = <value>;`) so the let-binding's own E503 obligation fires, then apply. | [#1017](https://github.com/aallan/vera/issues/1017) |
| Same-named helpers across imported fns | Two imported functions each with a same-named non-generic `where`-helper silently resolve to the FIRST one's body (check-green, wrong value) — imported bodies aren't parent-qualified. | Give where-helpers program-unique names across imported modules until #1015 lands. | [#1015](https://github.com/aallan/vera/issues/1015) |
| Same-named generic helpers under different parents | Two `forall` where-helpers sharing a name under *different* parent functions silently resolve to the FIRST one's body — the second parent's call runs the wrong helper (check-green, wrong value). | Give every generic where-helper a program-unique name until #1014 lands. | [#1014](https://github.com/aallan/vera/issues/1014) |
| Aunt-call in a generic's helper tree | A grandchild helper calling its parent's *sibling* helper under a `forall` ancestor fails `vera compile` with an internal unknown-func error (check-green). The same shape under a non-generic parent works. | Hoist the aunt's logic into the grandchild's parent, or call only own-children/siblings/ancestors' helpers from a generic's nested helpers. | [#1012](https://github.com/aallan/vera/issues/1012) |
| Private module generic called by a public one | Importing a module whose public `forall<T>` fn calls a **private** `forall<T>` sibling fails `vera compile` with an internal unknown-func error — private module generics are never harvested as clone bases. | Make the transitively-called module generic `public` (harvested since #774), or inline its body into the public generic. | [#1000](https://github.com/aallan/vera/issues/1000) |
