...
|---|---|---|---|
| Handler state slot | Referencing the state SLOT inside a `State<T>`-handled body (`@Int.0` under `handle[State<Int>](@Int = 0)`) silently reads an enclosing same-typed binding (a parameter or `let`) instead of the state — check-green, wrong value, no diagnostic — or fails compile with E699 when no same-typed binding encloses the handler. | Always read state via `get(())` inside the handled body; never via the state slot name. | [#973](https://github.com/aallan/vera/issues/973) |
...
| `@Nat` at return / value position | Narrowing `@Int` → `@Nat` at a function return or inside a value-position tuple/constructor is neither obligated nor guarded: a `verify`-clean function can return a negative `@Nat`. | Write `ensures(@Nat.result >= 0)` on narrowing returns; narrow via `let`-destructure (which IS obligated) instead of constructing in value position. | [#758](https://github.com/aallan/vera/issues/758) |
| `@Nat` through closures / mixed arms | `@Nat` → `@Int` widening through a closure argument/return/capture, or a `if`/`match` arm whose alternative is a genuine `@Int` slot, is silent (values above `2^63 - 1` reinterpret negative). | Keep `@Nat` values out of closure boundaries; keep `if`/`match` arms type-homogeneous (a non-negative literal alternative IS guarded). | [#820](https://github.com/aallan/vera/issues/820) |
| `verify --json` summary | On three examples the summary's `tier1_verified + tier3_runtime` exceeds `total` by 1. | Derive counts from the `obligations`/diagnostics lists, never the summary arithmetic. | [#967](https://github.com/aallan/vera/issues/967) |
