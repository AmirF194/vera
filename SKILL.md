...
| `show` of an alias-of-ADT value | `show`/`hash` of a value whose declared type is an alias of a whole ADT (`type MyBox = Box;` then `show(@MyBox.0)`) loud-skips the function (E602); never a wrong value. | Spell the parameter/binding with the ADT's own name (`@Box`) at show/hash sites, or convert before rendering. | [#1091](https://github.com/aallan/vera/issues/1091) |
