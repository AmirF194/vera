...
| Metric | Value |
|--------|-------|
| **Tests** | 7,582 across 122 files (~104,000 lines of test code; 7,481 passed + 26 stress, 75 skipped) |
...
| `test_checker_errors.py` | 56 | 865 | Error codes, resolution-coverage diagnostics, contracts, error accumulation (#420 split); cyclic type aliases incl. #1059 self-reference through a type argument (`Future<F>`, mutual `Future<B>`/`Future<A>`, `Array<L>`) rejected E132 |
...
| `test_codegen_arrays.py` | 88 | 1,510 | Byte type, array literals / bounds checking / length / range / concat, direct indexing of builtin call results (#1048), construction builtins (#209), compound element types (#132), array utilities (#419 split) |
| `test_array_map_slot_closure_1056.py` | 8 | 181 | #1056 — a fn-typed slot (`@Mapper.0` where `type Mapper = fn(A -> B)`) as the closure argument to `array_map` / `array_mapi`: let-bound and parameter slots, a type-changing `Int -> String` mapper, an `apply_fn`-parity pin, and the inline-`AnonFn` control |
