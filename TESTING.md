...
| Metric | Value |
|--------|-------|
| **Tests** | 7,904 across 127 files (~105,000 lines of test code; 7,801 passed + 26 stress, 77 skipped) |
| **Compiler code coverage** | 95% Python, 61% JavaScript — 91% combined (CI minimum: 80%) |
| **Conformance programs** | 161 programs across 9 spec chapters, validating every language feature |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (161 programs, see manifest.json)
...
| `test_types.py` | 81 | 430 | Type operations: subtyping, effect subtyping, equality, substitution, pretty-printing (including leaked-placeholder rendering to `?`, #1069), canonical names |
...
| `test_conformance.py` | 805 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 161 programs |
...
The conformance suite is a collection of 161 small, focused programs in `tests/conformance/` that systematically validate every language feature against the spec. Most programs are self-contained; the module-focused Chapter 8 cases use `import` statements where needed, and `ch07_cross_module_contracts.vera` still depends on `ch07_cross_module_contracts_lib.vera`. Each program tests one feature or a small group of related features.
...
├── ...                        # 161 programs total, organized by spec chapter
...
|--------|-------------------|
| `check_conformance.py` | All 161 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
...
| `check_conformance.py` | All 161 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
