...
| Metric | Value |
|--------|-------|
| **Tests** | 6,484 across 100 files (~79,000 lines of test code; 6,406 passed + 26 stress, 52 skipped) |
| **Compiler code coverage** | 95% Python, 61% JavaScript — 91% combined (CI minimum: 80%) |
| **Conformance programs** | 131 programs across 9 spec chapters, validating every language feature |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (131 programs, see manifest.json)
...
| `test_composite_postcondition_eq_912.py` | 15 | 420 | Composite (`Box`/`Option`/nested-ADT) `==` in an `ensures` postcondition lowers its runtime check to STRUCTURAL equality, not a pointer compare (#912): true `@T.result == ctor` postconditions run without a spurious `Postcondition violation` trap in both operand orders, the `E500`+runtime-trap negative controls prove a false composite postcondition is still rejected by `vera verify` AND traps, and a Tier-1 verify pin; round 2 — a function generic over the parameterized ADT itself (`fn id2(@Box<T> -> @Box<T>)`) with a slot-vs-slot postcondition compiles + runs via the free-type-var scalar fallback (with a Tier-3-not-Tier-1 soundness pin proving the fallback cannot resurrect #912), and a genuinely non-`Eq` contract composite (`Box<Array<Int>>`, `Tuple`) is a clean `E613` not an uncaught crash — mutation-validated (neuter the `ResultRef` arm → left-`@T.result` run tests flip RED; neuter the free-type-var routing → the `Box<T>` run tests flip RED; remove the postcondition backstop → the clean-`E613` tests raise an uncaught exception) |
...
| `test_conformance.py` | 655 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 131 programs |
...
| `run` | Compiles to WASM and executes correctly | 108 |
...
├── ...                        # 131 programs total, organized by spec chapter
