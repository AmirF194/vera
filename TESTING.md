...
| Metric | Value |
|--------|-------|
| **Tests** | 6,920 across 104 files (~93,500 lines of test code; 6,831 passed + 26 stress, 63 skipped) |
| **Compiler code coverage** | 95% Python, 61% JavaScript — 91% combined (CI minimum: 80%) |
| **Conformance programs** | 145 programs across 9 spec chapters, validating every language feature |
...
| **Contract verification** | 283 of 378 obligations (74.9%) across the 37 examples verified statically (Tier 1) — the denominator grew with the auto-synthesised primitive-op obligations of the soundness campaign; [#967](https://github.com/aallan/vera/issues/967) tracks a summary off-by-one on three examples |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (145 programs, see manifest.json)
...
| `test_cli.py` | 252 | 3,799 | CLI commands (check, verify, compile, run, serve, test, fmt, version, quiet), subprocess integration, JSON error paths, runtime traps, arg validation, multi-file resolution, IO exit codes, --explain-slots, `builtins`/`effects`/`errors` introspection dispatch, and a USAGE-completeness guard (every dispatched `cmd_<name>` handler has a help row) |
...
| `test_conformance.py` | 725 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 145 programs |
...
├── ...                        # 145 programs total, organized by spec chapter
...
| Metric | Value |
|--------|-------|
| **Tier 1 (static)** | 283 obligations — proved automatically by Z3 |
| **Tier 3 (runtime)** | 95 obligations — checked at runtime |
| **Total** | 378 obligations (74.9% static; the summary's `total` field disagrees by one on three examples — [#967](https://github.com/aallan/vera/issues/967)) |
...
|--------|-------------------|
| `check_conformance.py` | All 145 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
...
| `check_conformance.py` | All 145 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
