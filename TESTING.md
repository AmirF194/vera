...
| Metric | Value |
|--------|-------|
| **Tests** | 7,766 across 125 files (~104,000 lines of test code; 7,665 passed + 26 stress, 75 skipped) |
...
| **Contract verification** | 290 of 386 obligations (75.1%) across the 38 examples verified statically (Tier 1) — the denominator grew with the auto-synthesised primitive-op obligations of the soundness campaign |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (160 programs, see manifest.json)
python scripts/check_examples.py                     # 38 example programs
...
| `test_verifier_contracts.py` | 91 | 898 | Z3 verification over the example corpus, trivial/ensures/if-else/let/multi-clause contracts, counterexamples, tier classification, arithmetic, verification summaries, Diverge effect, edge cases, string-length + string-predicate verification (#839 split) |
| `test_verifier_nat_obligations.py` | 78 | 1,620 | **`@Nat` subtraction underflow obligation** (#520 — Path-A discharge via requires/path-conditions/path-aware Z3 refutation, pure-literal exclusion, Int-Int and Nat-Int exemptions) and **`@Nat` binding-site narrowing obligation** (#552/#747/#749 — Tier-1 `value >= 0` at let/call-arg/effect-op-arg/ctor-field/match-bind/destructure narrowing — a concrete site classifies `tier3_runtime` (codegen-guarded) while the effect-op argument and generic-instantiated constructor field classify `E504` (obligated but unguarded, #754/#757), walker-recursion pins, `_narrows_into_nat` verifier/codegen soundness parity; PR #972 clone-instantiated side-table substitution — a `Some(@T)` bind in an `Option<Nat>`-instantiated clone is no narrowing, genuine clone-path narrowings still obligated) (#839 split) |
...
| `test_verifier_calls_modules.py` | 60 | 1,514 | Call-site preconditions (incl. branch-aware), pipe-operator verification, cross-module contracts (#839 split) |
...
| `test_codegen_arrays.py` | 201 | 3,620 | Byte type, array literals / bounds checking / length / range / concat, direct indexing of builtin call results (#1048, #1051, alias-canonicalized #1055), `array_flatten` of inline nested literals + type-variable builtins nested as call arguments + alias-spelled / user-fn call-emission arguments incl. the Map/Set host-import tag inference (#1052, #1053, #1063), nested-alias element classification (#1067), generic-alias container returns (#1068), bare-alias returns + Block args (#1071), `Future<alias>` element sizing (#1074), construction builtins (#209), compound element types (#132), array utilities (#419 split) |
...
| `test_formatter.py` | 130 | 1,074 | Comment extraction, interior comment positioning, expression/declaration formatting, match arm block bodies, idempotency, parenthesization, spec rules, ability declarations |
...
| `test_conformance.py` | 800 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 160 programs |
...
| `check` | Parses and type-checks cleanly | 18 |
| `verify` | Type-checks and all contracts verified by Z3 | 13 |
| `run` | Compiles to WASM and executes correctly | 129 |

Almost all programs are at the `run` level — they compile and execute, producing correct results. Eighteen programs (`ch02_generic_over_unit_rejected`, `ch03_typed_holes`, `ch04_let_unit_rejected`, `ch05_apply_fn_arity`, `ch05_where_helper_outer_slot_rejected`, `ch07_cross_module_contracts_lib`, `ch07_handler_state_body_scope_rejected`, `ch07_state_unit_op_param_read_rejected`, `ch08_circular_import`, `ch08_cross_module_generic_lib`, `ch08_transitive_module_import_base`, `ch08_visibility_private`, `ch08_xmod_widen_lib`, `ch09_builtin_redefinition`, `ch09_eq_non_derivable_rejected`, `ch09_http`, `ch09_inference`, `ch09_ord_adt_rejected`) are at the `check` level. Eleven of them — `ch02_generic_over_unit_rejected`, `ch04_let_unit_rejected`, `ch05_apply_fn_arity`, `ch05_where_helper_outer_slot_rejected`, `ch07_handler_state_body_scope_rejected`, `ch07_state_unit_op_param_read_rejected`, `ch08_circular_import`, `ch08_visibility_private`, `ch09_builtin_redefinition`, `ch09_ord_adt_rejected`, and `ch09_eq_non_derivable_rejected` — are **negative tests** that assert a specific diagnostic (E206, E183, E201, E130, E130, E182, E011, E150, E151, E242, and E243 respectively) via the manifest's `expected_error` field; `ch09_http` and `ch09_inference` are environment-gated (network / API key). Thirteen programs (`ch03_slot_let_chains`, `ch03_slot_noncommutative`, `ch04_nested_option_ctor`, `ch04_primitive_obligations`, `ch05_apply_fn_typing`, `ch06_adt_sort_disambiguation`, `ch07_cross_module_contracts`, `ch07_io_read_char`, `ch07_io_sleep`, `ch07_random_effect`, `ch08_transitive_module_import_mid`, `ch09_http_server`, `ch09_math_builtins`) are at the `verify` level, using Z3-provable contracts.
...
├── ...                        # 160 programs total, organized by spec chapter
...
Across all 38 example programs (live sums of `vera verify --json`; regenerate by summing the `verification` block over `examples/*.vera`):

| Metric | Value |
|--------|-------|
| **Tier 1 (static)** | 290 obligations — proved automatically by Z3 |
| **Tier 3 (runtime)** | 96 obligations — checked at runtime |
| **Total** | 386 obligations (75.1% static; the summary's `total` field equals `tier1_verified + tier3_runtime`, derived from the reified obligation stream) |
...
| Ch 3: Slots | `@T.n` references, De Bruijn indexing | test_checker_*, test_codegen_* | ch03_slot_basic, ch03_slot_indexing, ch03_slot_result | all 38 examples |
...
Every one of the 38 example programs in `examples/` is tested through **every pipeline stage** via parametrised tests: parsing, AST transformation, type checking, contract verification, WASM compilation, and execution. If you add a new `.vera` example, it is automatically included in the round-trip suite.
...
|--------|-------------------|
| `check_conformance.py` | All 160 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
| `check_examples.py` | All 38 `.vera` examples pass `vera check` + `vera verify` |
...
| `check_conformance.py` | All 160 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
| `check_examples.py` | All 38 examples pass `vera check` + `vera verify` |
