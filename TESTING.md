...
=======
| **Tests** | 8,649 across 133 files (~106,000 lines of test code; 8,531 passed + 26 stress, 92 skipped) |
>>>>>>> c6ab3210 (Address the round-3 review: spec semantics, rollback totality, TCO pin)
...
| `test_checker_modules.py` | 47 | 1043 | Module-call diagnostics, cross-module typing, visibility enforcement, builtin redefinition (function E151 and effect E152 surfaced from a module into its importer), parsed module calls (#420 split) |
...
| `test_verifier_adt_decreases.py` | 20 | 733 | Match/ADT verification, decreases measures (incl. ADT decreases), mutual recursion (#839 split) |
...
=======
| `test_formatter.py` | 465 | 3,678 | Comment extraction, interior comment positioning, expression/declaration formatting, match arm block bodies, §1.8 rule 2 in value position (a `let`-bound `match`/`if` expands exactly as one in statement position, and a comment above an arm inside a statement's value stays on that arm), blank-line preservation (§1.8 rule 13 — gaps between statements, before a block result and around a comment, collapsed to one and never invented), idempotency, parenthesization, spec rules, ability declarations |
