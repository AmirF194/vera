...
| `test_array_map_slot_closure_1056.py` | 6 | 135 | #1056 — a fn-typed slot (`@Mapper.0` where `type Mapper = fn(A -> B)`) as the closure argument to `array_map` / `array_mapi`: let-bound and parameter slots, a type-changing `Int -> String` mapper, an `apply_fn`-parity pin, and the inline-`AnonFn` control |
