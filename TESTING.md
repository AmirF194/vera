...
| Metric | Value |
|--------|-------|
| **Tests** | 6,065 across 96 files (~79,000 lines of test code; 5,996 passed + 26 stress, 43 skipped) |
...
| `test_verifier_calls_modules.py` | 54 | 1294 | Call-site preconditions (incl. branch-aware and ADT-typed args, and #882 ensures-clause demotions), pipe-operator verification, cross-module contracts (#839 split) |
...
| `test_browser.py` | 112 | 2,403 | Browser parity: Python/wasmtime vs Node.js/JS-runtime output equivalence across IO, State, contracts, Markdown, Regex, and all compilable examples |
...
| `run` | Compiles to WASM and executes correctly | 92 |
...
|--------|-------------------|
| `check_conformance.py` | All 110 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
...
| `check_conformance.py` | All 110 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
