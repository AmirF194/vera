...
| Metric | Value |
|--------|-------|
| **Tests** | 6,336 across 99 files (~79,000 lines of test code; 6,260 passed + 26 stress, 50 skipped) |
| **Compiler code coverage** | 95% Python, 61% JavaScript — 91% combined (CI minimum: 80%) |
| **Conformance programs** | 125 programs across 9 spec chapters, validating every language feature |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (125 programs, see manifest.json)
...
| `test_checker_types.py` | 130 | 1655 | Primitive types, literals, binary/unary ops, generics, constructors, refinement types, arrays, tuples, return/match-arm types, byte-arithmetic + integer-literal-range rejection (#420 split), and the #898 cross-argument type-argument merge (`eq2(MkErr(5), MkOk("x"))` fully determines `Res<String, Int>` and type-checks; a per-parameter conflict `eq2(MkOk("x"), MkOk(5))` is a clear E205; a determined-non-Eq type still type-checks) |
...
| `test_conformance.py` | 625 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 125 programs |
...
| `run` | Compiles to WASM and executes correctly | 103 |
...
├── ...                        # 125 programs total, organized by spec chapter
...
|--------|-------------------|
| `check_conformance.py` | All 125 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
...
| `check_conformance.py` | All 125 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
