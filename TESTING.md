...
| Metric | Value |
|--------|-------|
| **Tests** | 8,620 across 131 files (~106,000 lines of test code; 8,485 passed + 26 stress, 90 skipped) |
...
| `test_checker_effects.py` | 84 | 1,311 | Effect declarations, abilities, effect subtyping, async effect, handler typing (#420 split), and the #1149 built-in-effect redeclaration gate (E152: divergent and faithful `effect IO`, codegen-only `Exn<E>`, a registry-parametrised sweep, and a differential pinning the gate's name set to what `vera effects --json` publishes) |
...
| `test_checker_modules.py` | 47 | 1038 | Module-call diagnostics, cross-module typing, visibility enforcement, builtin redefinition (function E151 and effect E152 surfaced from a module into its importer), parsed module calls (#420 split) |
...
| `test_cli.py` | 265 | 4,241 | CLI commands (check, verify, compile, run, serve, test, fmt, version, quiet), subprocess integration, JSON error paths (including the `verify --json` `obligations` array and its summary-reproducibility pin, #967), runtime traps, arg validation, multi-file resolution, IO exit codes, --explain-slots, `builtins`/`effects`/`errors` introspection dispatch, and a USAGE-completeness guard (every dispatched `cmd_<name>` handler has a help row) |
