...
| Metric | Value |
|--------|-------|
| **Tests** | 6,366 across 99 files (~79,000 lines of test code; 6,290 passed + 26 stress, 50 skipped) |
| **Compiler code coverage** | 95% Python, 61% JavaScript — 91% combined (CI minimum: 80%) |
| **Conformance programs** | 127 programs across 9 spec chapters, validating every language feature |
...
# Validation scripts
python scripts/check_conformance.py                  # conformance suite (127 programs, see manifest.json)
...
| `test_conformance.py` | 635 | 124 | Parametrized conformance suite: parse, check, verify, run, format idempotency across 127 programs |
...
| `run` | Compiles to WASM and executes correctly | 104 |
...
├── ...                        # 127 programs total, organized by spec chapter
...
|--------|-------------------|
| `check_conformance.py` | All 127 conformance entries hold at their declared level (parse/check/verify/run) — positives pass; the negatives fail `check` with their `expected_error` E-code |
...
| `check_conformance.py` | All 127 conformance entries hold at their declared level — positives pass; negatives fail `check` with their `expected_error` E-code |
