...
- **`vera serve` now appears in the CLI help (`vera` with no arguments) and the README command list** ([#305](https://github.com/aallan/vera/issues/305) follow-up).  The `serve` command — the HTTP host for a contract-verified `handle(Request -> Response)` program — shipped fully dispatched but was absent from the `USAGE` help text `vera` prints: the module header docstring listed it, but `USAGE` had drifted.  Its `Commands:` row and the `--port` / `--host` options are now listed, and a new `test_usage_lists_every_dispatched_command` guard fails if any future `cmd_<name>` handler is missing from `USAGE` — row-anchored, not a bare substring (the `--world server` row's description already contains "wasmtime serve", which a naive check would false-pass on).
...
[Unreleased]: https://github.com/aallan/vera/compare/v0.1.0...HEAD
