...
python scripts/check_conformance.py    # Verify all 138 conformance programs (positives pass their level; negatives fail with their expected_error E-code)
...
- `tests/conformance/` — 138 conformance programs validating every language feature against the spec
...
- All 138 conformance programs in `tests/conformance/` must hold at their declared level — positive entries pass, and the negative fixtures (`ch02_generic_over_unit_rejected`, `ch05_apply_fn_arity`, `ch08_circular_import`, `ch08_visibility_private`, `ch09_builtin_redefinition`, `ch09_ord_adt_rejected`) must *fail* `check` with their `expected_error` E-code
