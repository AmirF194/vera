...
python scripts/check_conformance.py    # Verify all 118 conformance programs (positives pass their level; negatives fail with their expected_error E-code)
...
- `tests/conformance/` — 118 conformance programs validating every language feature against the spec
...
- All 118 conformance programs in `tests/conformance/` must hold at their declared level — positive entries pass, and the negative fixtures (`ch05_apply_fn_arity`, `ch08_circular_import`, `ch08_visibility_private`, `ch09_builtin_redefinition`) must *fail* `check` with their `expected_error` E-code
