...
- Type aliases (`type Foo = ...`), effect declarations (`effect E { ... }`), module declarations, and import statements do not take visibility modifiers. These declarations are **module-local** — they are not importable by other modules. If another module needs the same type alias or effect, it must declare its own copy. The prelude declares no type or alias a program can name: its own declarations resolve through reserved names, and a type or alias whose name begins with `Vera` followed by an uppercase letter or digit is a compile error (**E154**) — whether the program *declares* that name or merely *mentions* it in a type. The prelude's internal namespace can therefore be neither re-typed nor referenced, and a program that wants a short name for a function type declares its own alias for it.
