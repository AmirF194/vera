...
An integer *literal* is always non-negative: a leading `-` is negation applied to the literal's magnitude, not part of it.  So the two ends of `Int`'s range are checked in different places.  The upper end is checked on the literal, against the bound its target implies (`2^63 - 1` for `Int`, `2^64 - 1` for `Nat`, `255` for `Byte`).  The lower end is checked on the negation, whose operand may reach the magnitude `2^63` — that is `Int`'s minimum `-9223372036854775808`, valid only in negated position — and no further: `-9223372036854775809` is **E149** at the negation, because the magnitude that would represent it exceeds what an `i64` can hold.  Either way the diagnostic names the value the program wrote.  The check reads the target's base type, so a refinement over one (`{ @Byte | @Byte.0 < 10 }`) is range-checked as its base and the predicate is left to verification.
