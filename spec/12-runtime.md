...
Two operations reach that identity by carrying a canonical form the specification states rather than by both hosts happening to agree. `json_stringify` emits the one form §9.7.1 pins, down to separators and number rendering, and refuses a non-finite number on both hosts instead of one refusing and the other substituting `null`. `md_render` emits the canonical Markdown §9.7.3 describes, collapsing a paragraph's soft line breaks and re-applying each container's prefix to every line of every child, which is what makes the render a fixed point and the §9.7.3 round-trip property hold on both hosts.
