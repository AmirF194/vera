...
|-----------|-----|---------|
| **Verification gaps that downgrade silently** | the effect-operation argument and the generic-instantiated constructor field have no codegen runtime guard, so an unverified compile can store a negative `@Nat` at one of those sites — every narrowing **binding site** is still statically obligated (#552, #747), but a narrowing at a function **return** position or in a value-position tuple/constructor component is not obligated even statically — that half is [#758](https://github.com/aallan/vera/issues/758), tracked as a bug in KNOWN_ISSUES | [#754](https://github.com/aallan/vera/issues/754), [#757](https://github.com/aallan/vera/issues/757) |
