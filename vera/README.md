...
**Parity enforcement:** `tests/test_browser.py` runs every compilable example, plus per-binding batteries over the Map/Set/Decimal/Json/Regex/Markdown host imports, through both Python/wasmtime and Node.js/JS-runtime, asserting identical stdout. Three cases are the exception, and deliberately so: `json_stringify` ([#1293](https://github.com/aallan/vera/issues/1293)) and `md_render` ([#1294](https://github.com/aallan/vera/issues/1294)) diverge between the hosts, so each runtime's exact output is pinned as its own string rather than compared — a fix on either side goes red until the pins are collapsed. `IO.read_file` / `IO.read_char` are pinned the same way, being browser stubs by design. Pre-commit hooks and CI trigger these tests on any change to the host binding surface.
